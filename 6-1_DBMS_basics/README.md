...
Минусы. 
1. Нет языка запросов, только команды. (При предыдущей сдаче д.з. намеренно не вклчючал данный пункт в недостатки, относя его к особенностям, т.к. система относится к NoSQL.) При разработке необходимо продумать все возможные обращения к данным и спроектировать к ним правильныепути доступа. Это снижает "гибкость", в сравнении с другими СУБД.
2. "Скудная" безопасность. Только базовая, на уровне инстанции (экземпляра).  
3. Т.к. Redis хранит данные в оперативной памяти, нет возможности управлять большим количеством данных, которые в неё не вместятся.  
4. Кластеры Redis поддерживают только одну базу данных, в отличие от других СУБД.  
5. Уникальный экземпляр Redis работает только на одном ядре процессора в однопоточном режиме. Поэтому он не считается масштабируемым.  
6. Не ясно, надо ли относить к недостаткам низкую отказоустойчивость в случае аппаратного сбоя сервера с Redis или сбоя самого Redis, из-за хранения базы в ОЗУ, учитывая, что Redis, всё же, предоставляет возможность организации кластера, в котором виртуальные серверы с Redis могут быть размещены на разных физических гипервизорах.
